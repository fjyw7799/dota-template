...
export function AddKeyBind(keyName: string, keydownCallback?: () => void, keyupCallback?: () => void) {
    const command = `${keyName}${Date.now()}`;

    if (keydownCallback) {
        Game.AddCommand(`+${command}`, keydownCallback, '', 0);
    }
    if (keyupCallback) {
        Game.AddCommand(`-${command}`, keyupCallback, '', 0);
    }
